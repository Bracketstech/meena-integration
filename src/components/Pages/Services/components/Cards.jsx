import React from 'react'

const Cards = ({arabic}) => {
  return (
    <div className="flex lg:gap-x-[2.5vw] lg:w-[40.625vw] flex-wrap justify-between lg:justify-normal">
                            <div
                                data-aos-anchor="#one"
                                data-aos="fade"
                                data-aos-delay={600}
                                className="w-[42.3076923077vw] lg:w-[19.0625vw] lg:mt-[4.16666666667vw] mt-[8.20512820513vw]"
                            >
                                <img
                                    src="/images/services/1.jpg"
                                    alt="Primary Care"
                                    className="w-full lg:rounded-[1.04166666667vw] rounded-[3.84615384615vw]"
                                />
                                <div className="lg:mt-[0.625vw] mt-[4%]">
                                    <span className="sm:text-[2.68292682927vw] sm:leading-[4.87804878049vw] lg:text-[1.14583333333vw] lg:leading-[1.82291666667vw] PingAR-Medium text-[4.10256410256vw] leading-[6.66666666667vw] text-[#3B3659]">
                                    {arabic ? "طب الأسرة": "Family medicine"}
                                    </span>
                                    <p className="sm:text-[2.19512195122vw] sm:leading-[3.65853658537vw] sm:mt-[1.46341463415vw] lg:text-[0.9375vw] lg:leading-[1.47569444444vw] text-[3.58974358974vw] leading-[6.05128205128vw] text-[#505050] PingAR-Light lg:mt-[0.41666666666vw] mt-[2.05128205128vw]">
                                        {arabic ? "لوريم ايبسوم دولار سيت أميت ,كونسيكتيتور أدايبا يسكينج أليايت,سيت دو أيوسمود تيمبور نكايديديونتيوت لابوري ات": "Lorem ipsum dolor sit amet, pscing consetetur sadipscing elitr, sed diam nonumy eirmod tempor invidunt ut"}
                                    </p>
                                </div>
                            </div>
                            <div
                                data-aos-anchor="#one"
                                data-aos="fade"
                                data-aos-delay={800}
                                className="w-[42.3076923077vw] lg:w-[19.0625vw]"
                            >
                                <img
                                    src="/images/services/2.jpg"
                                    alt="Primary Care"
                                    className="w-full lg:rounded-[1.04166666667vw] rounded-[3.84615384615vw]"
                                />
                                <div className="lg:mt-[0.625vw] mt-[4%]">
                                    <span className="sm:text-[2.68292682927vw] sm:leading-[4.87804878049vw] lg:text-[1.14583333333vw] lg:leading-[1.82291666667vw] PingAR-Medium text-[4.10256410256vw] leading-[6.66666666667vw] text-[#3B3659]">
                                    {arabic ? "صحة المرأة": "Women’s health"}
                                    </span>
                                    <p className="sm:text-[2.19512195122vw] sm:leading-[3.65853658537vw] sm:mt-[1.46341463415vw] lg:text-[0.9375vw] lg:leading-[1.47569444444vw] text-[3.58974358974vw] leading-[6.05128205128vw] text-[#505050] PingAR-Light lg:mt-[0.41666666666vw] mt-[2.05128205128vw]">
                                    {arabic ? "لوريم ايبسوم دولار سيت أميت ,كونسيكتيتور أدايبا يسكينج أليايت,سيت دو أيوسمود تيمبور نكايديديونتيوت لابوري ات": "Lorem ipsum dolor sit amet, pscing consetetur sadipscing elitr, sed diam nonumy eirmod tempor invidunt ut"}
                                        
                                    </p>
                                </div>
                            </div>
                            <div
                                data-aos="fade"
                                data-aos-delay={200}
                                className="w-[42.3076923077vw] lg:w-[19.0625vw] lg:mt-[4.16666666667vw] mt-[8.20512820513vw]"
                            >
                                <img
                                    src="/images/services/3.jpg"
                                    alt="Primary Care"
                                    className="w-full lg:rounded-[1.04166666667vw] rounded-[3.84615384615vw]"
                                />
                                <div className="lg:mt-[0.625vw] mt-[4%]">
                                    <span className="sm:text-[2.68292682927vw] sm:leading-[4.87804878049vw] lg:text-[1.14583333333vw] lg:leading-[1.82291666667vw] PingAR-Medium text-[4.10256410256vw] leading-[6.66666666667vw] text-[#3B3659]">
                                        Pediatric care
                                    </span>
                                    <p className="sm:text-[2.19512195122vw] sm:leading-[3.65853658537vw] sm:mt-[1.46341463415vw] lg:text-[0.9375vw] lg:leading-[1.47569444444vw] text-[3.58974358974vw] leading-[6.05128205128vw] text-[#505050] PingAR-Light lg:mt-[0.41666666666vw] mt-[2.05128205128vw]">
                                        {arabic ? "لوريم ايبسوم دولار سيت أميت ,كونسيكتيتور أدايبا يسكينج أليايت,سيت دو أيوسمود تيمبور نكايديديونتيوت لابوري ات": "Lorem ipsum dolor sit amet, pscing consetetur sadipscing elitr, sed diam nonumy eirmod tempor invidunt ut"}
                                    </p>
                                </div>
                            </div>
                            <div
                                data-aos="fade"
                                data-aos-delay={400}
                                className="w-[42.3076923077vw] lg:w-[19.0625vw]"
                            >
                                <img
                                    src="/images/services/4.jpg"
                                    alt="Primary Care"
                                    className="w-full lg:rounded-[1.04166666667vw] rounded-[3.84615384615vw]"
                                />
                                <div className="lg:mt-[0.625vw] mt-[4%]">
                                    <span className="sm:text-[2.68292682927vw] sm:leading-[4.87804878049vw] lg:text-[1.14583333333vw] lg:leading-[1.82291666667vw] PingAR-Medium text-[4.10256410256vw] leading-[6.66666666667vw] text-[#3B3659]">
                                        Dentistry
                                    </span>
                                    <p className="sm:text-[2.19512195122vw] sm:leading-[3.65853658537vw] sm:mt-[1.46341463415vw] lg:text-[0.9375vw] lg:leading-[1.47569444444vw] text-[3.58974358974vw] leading-[6.05128205128vw] text-[#505050] PingAR-Light lg:mt-[0.41666666666vw] mt-[2.05128205128vw]">
                                        {arabic ? "لوريم ايبسوم دولار سيت أميت ,كونسيكتيتور أدايبا يسكينج أليايت,سيت دو أيوسمود تيمبور نكايديديونتيوت لابوري ات": "Lorem ipsum dolor sit amet, pscing consetetur sadipscing elitr, sed diam nonumy eirmod tempor invidunt ut"}
                                    </p>
                                </div>
                            </div>
                            <div
                                data-aos="fade"
                                data-aos-delay={200}
                                className="w-[42.3076923077vw] lg:w-[19.0625vw] lg:mt-[4.16666666667vw] mt-[8.20512820513vw]"
                            >
                                <img
                                    src="/images/services/5.jpg"
                                    alt="Primary Care"
                                    className="w-full lg:rounded-[1.04166666667vw] rounded-[3.84615384615vw]"
                                />
                                <div className="lg:mt-[0.625vw] mt-[4%]">
                                    <span className="sm:text-[2.68292682927vw] sm:leading-[4.87804878049vw] lg:text-[1.14583333333vw] lg:leading-[1.82291666667vw] PingAR-Medium text-[4.10256410256vw] leading-[6.66666666667vw] text-[#3B3659]">
                                        General internal medicine
                                    </span>
                                    <p className="sm:text-[2.19512195122vw] sm:leading-[3.65853658537vw] sm:mt-[1.46341463415vw] lg:text-[0.9375vw] lg:leading-[1.47569444444vw] text-[3.58974358974vw] leading-[6.05128205128vw] text-[#505050] PingAR-Light lg:mt-[0.41666666666vw] mt-[2.05128205128vw]">
                                        {arabic ? "لوريم ايبسوم دولار سيت أميت ,كونسيكتيتور أدايبا يسكينج أليايت,سيت دو أيوسمود تيمبور نكايديديونتيوت لابوري ات": "Lorem ipsum dolor sit amet, pscing consetetur sadipscing elitr, sed diam nonumy eirmod tempor invidunt ut"}
                                    </p>
                                </div>
                            </div>
                            <div
                                data-aos="fade"
                                data-aos-delay={400}
                                className="w-[42.3076923077vw] lg:w-[19.0625vw]"
                            >
                                <img
                                    src="/images/services/6.jpg"
                                    alt="Primary Care"
                                    className="w-full lg:rounded-[1.04166666667vw] rounded-[3.84615384615vw]"
                                />
                                <div className="lg:mt-[0.625vw] mt-[4%]">
                                    <span className="sm:text-[2.68292682927vw] sm:leading-[4.87804878049vw] lg:text-[1.14583333333vw] lg:leading-[1.82291666667vw] PingAR-Medium text-[4.10256410256vw] leading-[6.66666666667vw] text-[#3B3659]">
                                        Dermatology
                                    </span>
                                    <p className="sm:text-[2.19512195122vw] sm:leading-[3.65853658537vw] sm:mt-[1.46341463415vw] lg:text-[0.9375vw] lg:leading-[1.47569444444vw] text-[3.58974358974vw] leading-[6.05128205128vw] text-[#505050] PingAR-Light lg:mt-[0.41666666666vw] mt-[2.05128205128vw]">
                                        {arabic ? "لوريم ايبسوم دولار سيت أميت ,كونسيكتيتور أدايبا يسكينج أليايت,سيت دو أيوسمود تيمبور نكايديديونتيوت لابوري ات": "Lorem ipsum dolor sit amet, pscing consetetur sadipscing elitr, sed diam nonumy eirmod tempor invidunt ut"}
                                    </p>
                                </div>
                            </div>
                            <div
                                data-aos="fade"
                                data-aos-delay={200}
                                className="w-[42.3076923077vw] lg:w-[19.0625vw] lg:mt-[4.16666666667vw] mt-[8.20512820513vw]"
                            >
                                <img
                                    src="/images/services/7.jpg"
                                    alt="Primary Care"
                                    className="w-full lg:rounded-[1.04166666667vw] rounded-[3.84615384615vw]"
                                />
                                <div className="lg:mt-[0.625vw] mt-[4%]">
                                    <span
                                        id="lineanimation2"
                                        className="sm:text-[2.68292682927vw] sm:leading-[4.87804878049vw] lg:text-[1.14583333333vw] lg:leading-[1.82291666667vw] PingAR-Medium text-[4.10256410256vw] leading-[6.66666666667vw] text-[#3B3659]"
                                    >
                                        Endocrine medicine
                                    </span>
                                    <p className="sm:text-[2.19512195122vw] sm:leading-[3.65853658537vw] sm:mt-[1.46341463415vw] lg:text-[0.9375vw] lg:leading-[1.47569444444vw] text-[3.58974358974vw] leading-[6.05128205128vw] text-[#505050] PingAR-Light lg:mt-[0.41666666666vw] mt-[2.05128205128vw]">
                                        {arabic ? "لوريم ايبسوم دولار سيت أميت ,كونسيكتيتور أدايبا يسكينج أليايت,سيت دو أيوسمود تيمبور نكايديديونتيوت لابوري ات": "Lorem ipsum dolor sit amet, pscing consetetur sadipscing elitr, sed diam nonumy eirmod tempor invidunt ut"}
                                    </p>
                                </div>
                            </div>
                            <div
                                data-aos="fade"
                                data-aos-delay={400}
                                className="w-[42.3076923077vw] lg:w-[19.0625vw]"
                            >
                                <img
                                    src="/images/services/8.jpg"
                                    alt="Primary Care"
                                    className="w-full lg:rounded-[1.04166666667vw] rounded-[3.84615384615vw]"
                                />
                                <div className="lg:mt-[0.625vw] mt-[4%]">
                                    <span className="sm:text-[2.68292682927vw] sm:leading-[4.87804878049vw] lg:text-[1.14583333333vw] lg:leading-[1.82291666667vw] PingAR-Medium text-[4.10256410256vw] leading-[6.66666666667vw] text-[#3B3659]">
                                        ENT
                                    </span>
                                    <p className="sm:text-[2.19512195122vw] sm:leading-[3.65853658537vw] sm:mt-[1.46341463415vw] lg:text-[0.9375vw] lg:leading-[1.47569444444vw] text-[3.58974358974vw] leading-[6.05128205128vw] text-[#505050] PingAR-Light lg:mt-[0.41666666666vw] mt-[2.05128205128vw]">
                                        {arabic ? "لوريم ايبسوم دولار سيت أميت ,كونسيكتيتور أدايبا يسكينج أليايت,سيت دو أيوسمود تيمبور نكايديديونتيوت لابوري ات": "Lorem ipsum dolor sit amet, pscing consetetur sadipscing elitr, sed diam nonumy eirmod tempor invidunt ut"}
                                    </p>
                                </div>
                            </div>
                            <div
                                data-aos="fade"
                                data-aos-delay={200}
                                className="w-[42.3076923077vw] lg:w-[19.0625vw] lg:mt-[4.16666666667vw] mt-[8.20512820513vw]"
                            >
                                <img
                                    src="/images/services/9.jpg"
                                    alt="Primary Care"
                                    className="w-full lg:rounded-[1.04166666667vw] rounded-[3.84615384615vw]"
                                />
                                <div className="lg:mt-[0.625vw] mt-[4%]">
                                    <span className="sm:text-[2.68292682927vw] sm:leading-[4.87804878049vw] lg:text-[1.14583333333vw] lg:leading-[1.82291666667vw] PingAR-Medium text-[4.10256410256vw] leading-[6.66666666667vw] text-[#3B3659]">
                                        Orthopedics
                                    </span>
                                    <p className="sm:text-[2.19512195122vw] sm:leading-[3.65853658537vw] sm:mt-[1.46341463415vw] lg:text-[0.9375vw] lg:leading-[1.47569444444vw] text-[3.58974358974vw] leading-[6.05128205128vw] text-[#505050] PingAR-Light lg:mt-[0.41666666666vw] mt-[2.05128205128vw]">
                                        {arabic ? "لوريم ايبسوم دولار سيت أميت ,كونسيكتيتور أدايبا يسكينج أليايت,سيت دو أيوسمود تيمبور نكايديديونتيوت لابوري ات": "Lorem ipsum dolor sit amet, pscing consetetur sadipscing elitr, sed diam nonumy eirmod tempor invidunt ut"}
                                    </p>
                                </div>
                            </div>
                            <div
                                data-aos="fade"
                                data-aos-delay={400}
                                className="w-[42.3076923077vw] lg:w-[19.0625vw]"
                            >
                                <img
                                    src="/images/services/10.jpg"
                                    alt="Primary Care"
                                    className="w-full lg:rounded-[1.04166666667vw] rounded-[3.84615384615vw]"
                                />
                                <div className="lg:mt-[0.625vw] mt-[4%]">
                                    <span className="sm:text-[2.68292682927vw] sm:leading-[4.87804878049vw] lg:text-[1.14583333333vw] lg:leading-[1.82291666667vw] PingAR-Medium text-[4.10256410256vw] leading-[6.66666666667vw] text-[#3B3659]">
                                        Opthalmology
                                    </span>
                                    <p className="sm:text-[2.19512195122vw] sm:leading-[3.65853658537vw] sm:mt-[1.46341463415vw] lg:text-[0.9375vw] lg:leading-[1.47569444444vw] text-[3.58974358974vw] leading-[6.05128205128vw] text-[#505050] PingAR-Light lg:mt-[0.41666666666vw] mt-[2.05128205128vw]">
                                        {arabic ? "لوريم ايبسوم دولار سيت أميت ,كونسيكتيتور أدايبا يسكينج أليايت,سيت دو أيوسمود تيمبور نكايديديونتيوت لابوري ات": "Lorem ipsum dolor sit amet, pscing consetetur sadipscing elitr, sed diam nonumy eirmod tempor invidunt ut"}
                                    </p>
                                </div>
                            </div>
                            {/* <div className="w-[42.3076923077vw] lg:w-[19.0625vw] lg:mt-[2.5vw] mt-[4.10256410256vw]">
                  <img src="/images/services/3.jpg" 
                  alt="Primary Care" className="w-full lg:h-[6.875vw] object-cover object-top lg:rounded-tr-[1.04166666667vw] rounded-tr-[3.84615384615vw]
                  lg:rounded-tl-[1.04166666667vw] rounded-tl-[3.84615384615vw] h-[15.641025641vw]">
                  
              </div>
              <div className="w-[42.3076923077vw] lg:w-[19.0625vw] lg:mt-[-1.66666666667vw] mt-[-4.10256410256vw]">
                  <img src="/images/services/4.jpg" 
                  alt="Primary Care" className="w-full lg:h-[11.0416666667vw] h-[23.5897435897vw] object-cover object-top lg:rounded-tr-[1.04166666667vw] rounded-tr-[3.84615384615vw]
                  lg:rounded-tl-[1.04166666667vw] rounded-tl-[3.84615384615vw]">
                  
              </div> */}
                        </div>
  )
}

export default Cards
